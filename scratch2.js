...
`;
content = content.replace("interface ConnectedPeer {", mockSocketDef + "\ninterface ConnectedPeer {");

// 3. Global socket parameter replacements (handling any whitespace)
content = content.replace(/socket\s*:\s*WebSocket/g, "socket: MockSocket");
content = content.replace(/_socket\s*:\s*WebSocket/g, "_socket: MockSocket");
content = content.replace(/existingSocket\s*:\s*WebSocket/g, "existingSocket: MockSocket");
content = content.replace(/WebSocket\.OPEN/g, "WEBSOCKET_OPEN");
content = content.replace(/Map<WebSocket/g, "Map<MockSocket");
content = content.replace(/socket\s*===\s*WebSocket/g, "socket === MockSocket"); // just in case
content = content.replace(/existingSocket\s*:\s*WebSocket\s*\|\s*null/g, "existingSocket: MockSocket | null");

// 4. Map replacements and config additions
content = content.replace("private readonly peers: Map<MockSocket, ConnectedPeer> = new Map();", "private peers: Map<MockSocket, ConnectedPeer> = new Map();\n  private peersById: Map<string, MockSocket> = new Map();");
content = content.replace("private readonly rateLimiters: Map<MockSocket, RateLimiterEntry> = new Map();", "private rateLimiters: Map<MockSocket, RateLimiterEntry> = new Map();");
...
const handleDisconnectRegex = /private async handleDisconnect\(socket: MockSocket\): Promise<void> \{[\s\S]*?this\.rateLimiters\.delete\(socket\);\n  \}/;
